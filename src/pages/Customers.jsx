...
const TIER = {
  clear: { dot: 'bg-kirana-500', text: 'text-kirana-600', bg: 'bg-kirana-50', ring: 'ring-kirana-100' },
  green: { dot: 'bg-kirana-500', text: 'text-kirana-600', bg: 'bg-kirana-50', ring: 'ring-kirana-100' },
  amber: { dot: 'bg-saffron-500',   text: 'text-saffron-600',   bg: 'bg-saffron-50',   ring: 'ring-saffron-100' },
  red:   { dot: 'bg-khaata-500',  text: 'text-khaata-600',  bg: 'bg-khaata-50',  ring: 'ring-khaata-100' },
}
      ...
      {showAdd && (
        <div className="card-elevated space-y-4 animate-slide-up">
          <div className="flex items-center justify-between">
            <p className="font-bold text-ink-700 flex items-center gap-2">
              <span className="w-7 h-7 rounded-xl bg-saffron-50 flex items-center justify-center">
                <BookOpen size={14} className="text-saffron-500" />
              </span>
              Naya Customer
            </p>
            <button onClick={() => setShowAdd(false)} className="w-8 h-8 flex items-center justify-center rounded-xl text-ink-400 hover:text-ink-600 hover:bg-cream-100 transition-colors">
              <X size={16} />
            </button>
          </div>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-ink-400">WhatsApp Number</label>
            <div className="flex gap-2">
              <div className="flex items-center justify-center bg-cream-50 border border-cream-200 rounded-xl px-3 text-ink-400 text-sm font-semibold w-16 flex-shrink-0">+91</div>
              <input
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-ink-400">Notes</label>
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          ...
          className={`px-3 py-2 rounded-xl text-xs font-bold transition-colors ${
            !onlyUdhaar ? 'bg-ink-700 text-white' : 'bg-white border border-cream-200 text-ink-400'
          }`}
        >
        ...
        <button
          ...
          className={`px-3 py-2 rounded-xl text-xs font-bold transition-colors ${
            onlyUdhaar ? 'bg-saffron-500 text-white' : 'bg-white border border-cream-200 text-ink-400'
          }`}
        >
      ...
      {filtered.length === 0 && (
        <div className="empty-state">
          <div className="empty-state-icon">
            <BookOpen size={28} strokeWidth={1.4} className="text-ink-300" />
          </div>
            ...
            <div key={cust.id} className="card p-0 overflow-hidden">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-ink-700 text-sm truncate">{cust.name}</p>
                ...
                <div className="text-right flex-shrink-0">
                  {udhaar > 0 ? (
                    ...
                    <p className="text-xs font-semibold text-kirana-500">Saaf ✓</p>
                  )}
                </div>
              </button>
              ...
              {open && (
                <div className="px-4 pb-4 space-y-3 border-t border-cream-50 bg-cream-50/40">

                  {/* Edit mode */}
                  {isEditing ? (
                    <div className="space-y-2 pt-3">
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold text-ink-400 uppercase">Naam</label>
                        <input className="input-field py-2 text-sm" value={editing.name}
                          onChange={e => setEditing(p => ({ ...p, name: e.target.value }))} />
                      </div>
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold text-ink-400 uppercase">Phone</label>
                      ...
                      <div className="flex gap-2 pt-1">
                        <button onClick={saveEdit}
                          className="flex-1 py-2.5 bg-kirana-500 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 active:scale-95 transition-transform">
                          <Save size={13} /> Save
                        </button>
                        <button onClick={() => setEditing(null)}
                          className="px-4 py-2.5 bg-white border border-cream-200 rounded-xl text-xs font-bold text-ink-600 active:scale-95 transition-transform">
                          Cancel
                        </button>
                      </div>
                    </div>
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-2 pt-3">
                        ...
                        {udhaar > 0 && (
                          <button onClick={() => paisaAaya(cust, false)}
                            className="px-3 py-2 bg-kirana-500 text-white rounded-xl text-xs font-bold whitespace-nowrap active:scale-95 transition-transform">
                            Paisa Aaya
                          </button>
                        )}
                        <button onClick={() => moreUdhaar(cust)}
                          className="px-3 py-2 bg-saffron-50 text-saffron-600 rounded-xl text-xs font-bold whitespace-nowrap active:scale-95 transition-transform">
                          + Udhaar
                        </button>
                      </div>
                      ...
                      <div className="flex gap-2">
                        {udhaar > 0 && cust.phone && (
                          <button onClick={() => yaadDilao(cust)}
                            className="flex-1 py-2.5 bg-saffron-50 text-saffron-600 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 active:scale-95 transition-transform">
                            <MessageCircle size={13} /> Yaad Dilao
                          </button>
                        )}
                        {udhaar > 0 && (
                          <button onClick={() => paisaAaya(cust, true)}
                            className="flex-1 py-2.5 bg-kirana-50 text-kirana-600 rounded-xl text-xs font-bold active:scale-95 transition-transform">
                            Pura ₹{udhaar} aaya
                          </button>
                        )}
                        {cust.phone && udhaar <= 0 && (
                          <a href={waLink(cust.phone, `Namaste ${cust.name} ji! 🙏`)} target="_blank" rel="noreferrer"
                            className="flex-1 py-2.5 bg-kirana-50 text-kirana-600 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 active:scale-95 transition-transform">
                            <Phone size={13} /> WhatsApp
                          </a>
                        )}
                        <button onClick={() => startEdit(cust)}
                          className="px-3 py-2.5 bg-white border border-cream-200 text-ink-600 rounded-xl text-xs font-bold flex items-center gap-1 active:scale-95 transition-transform"
                          title="Edit details">
                          <Edit2 size={13} />
                        </button>
                      </div>
                      ...
                      {cust.notes && (
                        <p className="text-xs text-ink-400 italic bg-white rounded-xl px-3 py-2">{cust.notes}</p>
                      )}
                    </>
                  )}
                </div>
