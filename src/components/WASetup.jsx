...
export default function WASetup() {
  ...
  if (phase === 'connected') {
    return (
      <Section>
      <div className="card flex items-center gap-3 border-kirana-100 bg-kirana-50/40">
        <div className="w-8 h-8 rounded-xl bg-kirana-100 flex items-center justify-center flex-shrink-0">
          <Wifi size={16} className="text-kirana-600" />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-ink-700">WhatsApp Connected</p>
          <p className="text-xs text-ink-400 mt-0.5">Orders are being ingested automatically</p>
        </div>
        <CheckCircle size={18} className="text-kirana-500 flex-shrink-0" />
      </div>
      </Section>
    )
  }
  ...
  if (phase === 'qr') {
    return (
      <Section>
      <div className="card space-y-3 border-kirana-100">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-xl bg-kirana-50 flex items-center justify-center flex-shrink-0">
            <Wifi size={14} className="text-kirana-500" />
          </div>
          <p className="font-bold text-ink-700 text-sm">Scan to Connect WhatsApp</p>
        </div>
        ...
        {qrSrc && (
          <img
            ...
            className="w-48 h-48 mx-auto rounded-xl border border-ink-100"
          />
        )}
  ...
  if (phase === 'loading') {
    return (
      <Section>
      <div className="card flex items-center gap-3 border-ink-100">
        <RefreshCw size={18} className="text-kirana-500 animate-spin flex-shrink-0" />
        <div>
          <p className="text-sm font-bold text-ink-700">Starting WhatsApp…</p>
          <p className="text-xs text-ink-400 mt-0.5">QR code will appear in a moment</p>
        </div>
      </div>
      </Section>
    )
    ...
    return (
      ...
      <div className="card space-y-2 border-red-100 bg-red-50/30">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-bold text-ink-700">WhatsApp setup failed</p>
        </div>
        <p className="text-xs text-ink-400">{errMsg || 'Make sure the backend is running'}</p>
        <button onClick={startSetup} className="text-xs font-semibold text-kirana-700 bg-kirana-50 px-3 py-2 rounded-xl w-full">
          Retry
        </button>
      </div>
      </Section>
  ...
  return (
    ...
    <div className="card space-y-3 border-dashed">
      <div className="flex items-center gap-2">
        <div className="w-7 h-7 rounded-xl bg-cream-100 flex items-center justify-center flex-shrink-0">
          <WifiOff size={14} className="text-ink-400" />
        </div>
        <p className="font-bold text-ink-700 text-sm">Auto-Ingest WhatsApp Orders</p>
      </div>
      <p className="text-xs text-ink-400 leading-relaxed">
