...
If the catalog contains any of:
  Loose Turmeric Powder 100g (with alias "haldi" or "turmeric")
  Catch Turmeric Powder 100g
  Patanjali Haldi 50g

then "haldi" / "haldi powder" / "ek pao haldi" / "हल्दी" → match the haldi/turmeric product (qty=1).

Same pattern for: dudh→milk, anda→egg (only if egg in catalog), namak→salt, cheeni→sugar,
chai→tea, aata→atta/wheat-flour, chawal→rice, dal→lentils, mirch→chilli,
dhaniya→coriander (powder is acceptable if leaves not available).

Brand abbreviations always resolve to the same brand:
  P-G / PG → Parle-G    (NOT Britannia Pure Magic, NOT Britannia Pure Gold)
  Mggi / magi → Maggi
  A milk / amul dudh → Amul Milk (the plain milk variant, NOT Basundi/Lassi/Dahi/Butter)
