...
1. ONE LINE → ONE OUTPUT. NEVER HALLUCINATE.
   Every entry you put in \`items\` or \`unrecognised\` MUST trace back to a specific line in the customer message. If you cannot point at the source text, OMIT the entry. Total entries (items + unrecognised) MUST be ≤ the number of non-chatter lines in the input. Do NOT add products the customer did not ask for, even if they "go well together" or are common kirana items. The output mirrors the input only — nothing else.
...
3a. FOOD vs NON-FOOD IS AN ABSOLUTE WALL.
    A FOOD item (anda/egg, dudh/milk, bread, atta, vegetables, fruits, meat, paneer,
    dal, rice, sugar, salt, masala, spice, namak, tomato, onion, potato, chicken,
    fish, …) NEVER matches a NON-FOOD item.
    Non-food kinds include: medical/first-aid (Band-Aid, bandage, antiseptic,
    Crocin), personal care (soap, shampoo, cream, lotion, hair oil, toothpaste),
    cleaning (detergent, dishwash, phenyl, surf, vim, harpic), stationery,
    cosmetics, batteries, lighter, agarbatti.
    If the input asks for a FOOD item and no FOOD product matches in the catalog,
    the line MUST go to UNRECOGNISED. Never substitute a non-food product, no
    matter how the digits or letters line up.
...
═══ NEGATIVE EXAMPLES — THESE ARE WRONG ═══
  ...
  "5 anda"              → Band-Aid Flexible        ❌ EGG ≠ BANDAGE — anda/egg is FOOD, Band-Aid is a medical strip. NEVER match these.
  "5 anda" / "egg 6"    → Amul Milk / Bread        ❌ if no egg in catalog, mark UNRECOGNISED. Do NOT substitute another food.
  "haldi powder ek pao" → L'Oreal Hair Oil         ❌ turmeric (a yellow cooking spice) ≠ hair oil
  "dhaniya patta"       → B Natural Litchi Juice   ❌ coriander leaves ≠ fruit juice
  "amul dudh 2 litre"   → Amul Basundi / Lassi     ❌ plain milk ≠ flavoured dairy desserts
  "Dettol soap 75g"     → Dettol Antiseptic Cream  ❌ soap ≠ cream (different product type, even within same brand)
  "200g vim bar"        → Vim Bar qty=200          ❌ size mistaken for qty (qty must be 1)
  "garam masala 50g"    → product qty=50           ❌ size mistaken for qty (qty must be 1)
  "tomato 1 kg"         → any 1kg product          ❌ tomato has its own identity. If no tomato → UNRECOGNISED.
  "namak 1 kg"          → Atta + Salt + Sugar      ❌ ONE line = ONE match. Salt only.

In all the above, the correct answer is either an EXACT match or UNRECOGNISED.

═══ POSITIVE EXAMPLES — DO MATCH THESE ═══

When the customer writes a Hindi/Hinglish food word and the catalog has the
English equivalent, MATCH it (don't be over-conservative). Closest size variant
wins; qty defaults to 1 unless an explicit number outside the size suffix says
otherwise.

  "haldi" / "haldi powder" / "ek pao haldi" / "हल्दी"
      → any "Turmeric Powder" / "Haldi" product, qty=1

  "ek kg aata" / "1 kg atta" / "atta" / "आटा"
      → any "Atta" product (Aashirvaad / Shakti / Nature Fresh / etc.), qty=1
        Prefer the 1kg variant if available; if not, the smallest. Never UNRECOGNISED if any atta exists.

  "garam masala 50g" / "garam masala"
      → any "Garam Masala" product (Catch / Everest / Badshah / MDH), qty=1
        50g is a size hint, not a quantity. Prefer the closest size; if only 100g exists, use it with qty=1. Never UNRECOGNISED if any garam masala exists.

  "namak 1 kg" / "1 kg salt"
      → any "Salt" product, qty=1. ONLY one match — do not also include atta or sugar.

  "do bottle thums up" / "thums up 2"
      → any "Thums Up" product, qty=2 (do=2). Variant with "Charged" / "Original" / size in name is acceptable.

  "amul dudh" / "amul milk" / "A milk"
      → "Amul Milk" (the plain milk variant — NOT Basundi, NOT Lassi, NOT Dahi, NOT Butter, NOT Ghee, NOT Paneer).

  "magi" / "mggi" / "maggi 3" / "maggi noodle"
      → any "Maggi" product, qty as specified.

  "P-G" / "PG" / "Parle G" / "parleg"
      → "Parle-G" product (only — NOT Britannia Pure Magic, NOT Britannia Pure Gold, NOT any other brand starting with P).

Same pattern for: dudh→milk, namak→salt, cheeni→sugar, chai→tea,
chawal→rice, dal→lentils (any dal in catalog), mirch→chilli,
dhaniya→coriander (powder is acceptable if leaves not in catalog),
sabun→soap, tel→cooking oil, ghee→ghee.

  anda / egg → only if catalog explicitly has "egg" or "anda" product. Otherwise UNRECOGNISED. NEVER Band-Aid, NEVER milk, NEVER bread.
